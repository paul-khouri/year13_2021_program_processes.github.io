...
class Slider extends InteractiveObject{
    ...
    constructor(x,y,w,h,fill, stroke, strokeWidth,lable, minV,maxV, fraction=false){
...
// because we need to add to it
    ...
    getValue(){
        if(this.fraction){
            return Math.round((this.minV +(this.maxV-this.minV)*(this.x_s - this.x)/this.w)*100)/100
        }else {
    ...
    draw(){
        this.basicRect(this.x, this.y, this.w, 4*this.r, this.fill)
        this.centredText(this.lable, this.x +this.w/2, this.y+this.r, this.stroke)
        this.drawLine(this.x, this.y_s, this.x + this.w, this.y_s, this.stroke)
        let fill = this.fill
        if(Slider.taken === this){
            fill = "rgb(255,255,255)"
        }
        this.drawCircle(this.x_s, this.y_s, this.r, fill, this.stroke, this.strokeWidth)
        this.centredText(this.value.toString(), this.x_s, this.y_s, this.stroke)
    }
...
Slider.prototype.basicRect = basicRect
