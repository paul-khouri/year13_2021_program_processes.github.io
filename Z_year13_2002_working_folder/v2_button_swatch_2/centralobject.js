...
class Centre extends InteractiveObject{
    ...
    get_colours(){
        ...
        let colour_data ={shapeName: chosenShape, f:fillColour,s:strokeColour}
        return colour_data
    ...
    mDown(e) {
        ...
        if(this.okayForDraw && this.chosenShape === "Brush1"){
            ...
            let fillColour=this.get_colours().f

            var digits= fillColour.match( /\d+(?=,)|(\d+(\.\d+))+/g);

            let temp = new BlurryCircle(fillColour)
            this.objectSet.push(temp)
        }else if(this.okayForDraw && this.chosenShape === "Brush2"){
            this.brush = true

            let fillColour=this.get_colours().f
            let temp = new Spatter(fillColour)
            this.objectSet.push(temp)
        }else if(this.okayForDraw && this.chosenShape === "Brush3"){
            this.brush = true
            let fillColour=this.get_colours().f
            let temp = new SuperSpatter(fillColour)
            this.objectSet.push(temp)
        }
