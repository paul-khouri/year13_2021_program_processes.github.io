...
class BlurryCircle extends InteractiveObject{
    ...
    update(){
        if(this.mouseIsDown && !this.completed) {
            this.blurredCircle(this.xMouse, this.yMouse,10,this.colour, ctx_s)
            ctx.drawImage(canvasSecond, 0, 0, width, height);
        }
        if(this.completed){
            this.brushImage.update()
...
function blurredCircle(x,y,r, col="rgb(255,255,255)",ct=ctx){
    // Create a radial gradient
// The inner circle is at x=110, y=90, with radius=30
// The outer circle is at x=100, y=100, with radius=70
    let digits = col.match(/\d+/g).map(Number);
    console.log(digits)
    let gradient = ct.createRadialGradient(x,y,0, x,y, r);
    gradient.addColorStop(0, `rgba(${digits[0]},${digits[1]},${digits[2]},1)`);
    gradient.addColorStop(1, `rgba(${digits[0]},${digits[1]},${digits[2]},${0})`);
