...
update(){
    ...
    for(var i = 0 ; i < this.ball.length ; i ++){
        ...
        var xCNext = xC + this.ball[i].getSpeed()*Math.cos(this.ball[i].getTravelAngle());
        var yCNext = yC + this.ball[i].getSpeed()*Math.sin(this.ball[i].getTravelAngle());
/*
        if(xC >= xMin+R && xCNext <= xMin + R  || xC <= xMax-R && xCNext >= xMax - R ){
            this.ball[i].setTravelAngle( Math.PI-this.ball[i].getTravelAngle());
            }

        if(yC >= yMin+R && yCNext <= yMin+R   || yC <= yMax-R && yCNext >= yMax -R){
            this.ball[i].setTravelAngle( 2*Math.PI-this.ball[i].getTravelAngle());
            }
            */
            this.boundaryReflect(xC,yC,xMin, xMax, yMin, yMax , R, xCNext, yCNext, this.ball[i]);
            ...
            if( (d >= rC + R  && dNext <= rC + R) || (d <= rC - R  && dNext >= rC - R)    ){
                //collison
                if( this.bounce[j].getOpen(angle) == false){
                this.boundaryReflect(xC,yC,xMin, xMax, yMin, yMax , R, xCNext, yCNext, this.ball[i]);
                this.ball[i].setTravelAngle(reflectAngle);
                xCNext = xC + this.ball[i].getSpeed()*Math.cos(this.ball[i].getTravelAngle());
                yCNext = yC + this.ball[i].getSpeed()*Math.sin(this.ball[i].getTravelAngle());
                this.boundaryReflect(xC,yC,xMin, xMax, yMin, yMax , R, xCNext, yCNext, this.ball[i]);
            }
                //console.log(Math.round(reflectAngle*180/Math.PI))
            }

            //console.log(this.bounce[i].getOpen(angle))

            if( this.bounce[j].getOpen(angle)){
...
boundaryReflect(xC, yC, xMin, xMax, yMin, yMax , R, xCNext, yCNext, b  ){
    if(xC >= xMin+R && xCNext <= xMin + R  || xC <= xMax-R && xCNext >= xMax - R ){
        b.setTravelAngle( Math.PI-b.getTravelAngle());
        }

    if(yC >= yMin+R && yCNext <= yMin+R   || yC <= yMax-R && yCNext >= yMax -R){
       b.setTravelAngle( 2*Math.PI-b.getTravelAngle());
        }
...
drawDiagram(xC, yC, xCircle, yCircle,rC, angle, reflectAngle, i){
        ...
        this.drawLine(xC,yC,xCircle,yCircle,0.1);
        ...
        // tangent line
        this.drawLine(xR+ rC*Math.cos(perpAng), yR + rC*Math.sin(perpAng), xR- rC*Math.cos(perpAng), yR - rC*Math.sin(perpAng),0.1 );
        // vector of ball[0] drawn at tangent line
        this.drawLine(xR, yR, xR - rC*Math.cos(this.ball[i].getTravelAngle()), yR - rC*Math.sin(this.ball[i].getTravelAngle()),0.1 );
       
        this.drawLine(xR, yR, xR +rC*Math.cos(reflectAngle), yR + rC*Math.sin(reflectAngle), 0.1 );
        this.writeText(angle, xCircle, yCircle);
