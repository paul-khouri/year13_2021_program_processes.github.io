...
if( SFButton.selectedStyle =="Fill"){
    ...
    if(this.selectedStyle != "Stroke"){
        // initiated on first click to reset to previous colour value
        var fc = this.strokeColour.match(/[. 0-9]+/g).map(Number);
        this.R.setValue(fc[0]);
        this.G.setValue(fc[1]);
        this.B.setValue(fc[2]);
        this.A.setValue(fc[3]*100);
...
// colour rectangle
    ...
    ctx.beginPath();
    ctx.rect(this.x+ 4*0.125*this.w,this.y+0.1*this.h,2*0.125*this.w,0.8*this.h);
    ...
    ctx.lineWidth = 1;
    ctx.beginPath();
    ctx.rect(this.x,this.y-0.2*this.h,7*0.125*this.w,this.h+0.2*this.h);
    ctx.stroke();*/
}
