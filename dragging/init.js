...
draw_grid(){
    var grid_interval = this.interval;
    for(var i=0; i< width/grid_interval; i++){
        this.draw_line(i*grid_interval,0,i*grid_interval,height);
    }
    for(var i=0; i< height/grid_interval; i++){
        this.draw_line(0,i*grid_interval,width,i*grid_interval);
    }
}

draw_line(x_1, y_1, x_2,y_2){
    this.updateContext({s:"rgb(255,255,255)", l:0.25})
    ctx.beginPath();
    ctx.moveTo(x_1, y_1);
    ctx.lineTo(x_2,y_2);
    ctx.stroke();
